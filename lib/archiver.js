...
function installDefaults(obj) {
  ...
  obj.defineAction('normalize-file', function(state, args, overlay) {
    ...
    parts = _.defaults(overlay ? _.cloneDeep(overlay) : {}, parts);
    parts.update();
    ...
    var destFile = path.join(path.dirname(state.file), parts.filename);
    if ( fs.renameSync(state.file, destFile) ) {
      state.file = destFile;
      return state;
    }
    return false;
...
_.assign(Archiver.prototype, {
  ...
  runTest: function(test, file, extra) {
    if ( _.get(this.tests, test.test, null) !== null ) {
      return this.tests[test.test].call(this, _.cloneDeep(test), file, extra);
    }
  ...
  runAction: function(state, action, extra) {
    if ( _.get(this.actions, action.action, null) !== null ) {
      ...
      return this.actions[action.action].call(this, state, _.cloneDeep(action), extra);
    }
  ...
  exec: function(rule) {
    glob(rule.src, {}, function(err, files) {
      ...
      _.forEach(files, function(file) {
        ...
        var passed = _.map(r.match, function(test) {
          return this.runTest(test, file);
        }, this);

        // console.log('Passed:', passed);
        if ( _.compact(passed).length == r.match.length ) {
          var state = {
            originalFile : file,
            file         : file
          };

          _.forEach(r.run, function(action) {
